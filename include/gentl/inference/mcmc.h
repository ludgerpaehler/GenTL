...
double mh_accept_prob(double model_log_weight, double proposal_forward_score,
                      double proposal_backward_score) {
  return std::min(1.0, std::exp(model_log_weight + proposal_backward_score -
                                proposal_forward_score));
}
...
concept MHProposalCompatible = requires(ModelTrace trace,
                                        ProposalTrace proposal_trace,
                                        std::mt19937 rng) {
  {trace.update(rng, gentl::change::no_change, proposal_trace.choices(),
                UpdateOptions())};
};
#endif
...
#ifdef __cpp_concepts
requires InPlaceSimulatableGenerativeFunction<Proposal, ModelTrace,
                                              ProposalParams, ProposalTrace> &&
    MHProposalCompatible<ModelTrace, ProposalTrace> && Revertible<ModelTrace>
#endif
bool mh(RNGType &rng, ModelTrace &model_trace, const Proposal &proposal,
        ProposalParams &proposal_params, ProposalTrace &proposal_trace,
        bool precompute_gradient = false) {
  proposal(model_trace)
      .simulate(rng, proposal_params, SimulateOptions(), proposal_trace);
  ...
  double model_log_weight = model_trace.update(
      rng, gentl::change::no_change, forward_constraints,
  ...
  auto [proposal_retval, proposal_backward_score] =
      proposal(model_trace).assess(rng, proposal_params, backward_constraints);
  double prob_accept = mh_accept_prob(model_log_weight, proposal_forward_score,
                                      proposal_backward_score);
...
template <typename ProposalTrace, typename ModelTrace, typename Proposal,
          typename ProposalParams, typename RNGType>
#ifdef __cpp_concepts
requires SimulatableGenerativeFunction<Proposal, ModelTrace, ProposalParams,
                                       ProposalTrace> &&
    MHProposalCompatible<ModelTrace, ProposalTrace> && Revertible<ModelTrace>
#endif
bool mh(RNGType &rng, ModelTrace &model_trace, const Proposal &proposal,
        ProposalParams &proposal_params, bool precompute_gradient = false) {
  auto proposal_trace =
      proposal(model_trace).simulate(rng, proposal_params, SimulateOptions());
  double proposal_forward_score = proposal_trace->score();
  const auto &forward_constraints = proposal_trace->choices();
  double model_log_weight = model_trace.update(
      rng, gentl::change::no_change, forward_constraints,
...
double mala_propose_values(ChoiceBuffer &proposal, const ChoiceBuffer &current,
                           const ChoiceBuffer &gradient, double tau,
                           RNGType &rng) {
  ...
  double log_density =
      (current.cend() - current.cbegin()) * (-std::log(stdev) - logSqrt2Pi);

  // first compute the mean in-place
  proposal = current + (tau * gradient);

  // then sample new values (over-writing the mean), and finish computing the
  // log density
  for (auto proposal_it = proposal.begin(); proposal_it != proposal.end();
       proposal_it++) {
    double standard_normal_increment = standard_normal(rng);
    *proposal_it += (standard_normal_increment * stdev);
    log_density +=
        -0.5 * (standard_normal_increment * standard_normal_increment);
  }
  return log_density;
}
...
double mala_assess(const ChoiceBuffer &proposed, const ChoiceBuffer &current,
  ...
  for (auto proposed_it = proposed.cbegin(); proposed_it != proposed.cend();
       proposed_it++) {
    double standard_normal_increment =
        (*proposed_it - *(proposal_mean_it++)) / stdev;
    log_density +=
        -0.5 * (standard_normal_increment * standard_normal_increment);
  }
  return log_density;
...
template <typename Trace, typename Selection, typename RNG,
          typename ChoiceBuffer>
#ifdef __cpp_concepts
requires TopLevelGradientBasedChoiceUpdates<Trace, Selection, ChoiceBuffer> &&
    Revertible<Trace>
#endif
bool mala(Trace &trace, const Selection &selection, double tau,
  ...
  double forward_log_density =
      mala_propose_values(proposed_values, trace.choices(selection),
                          trace.choice_gradient(selection), tau, rng);
  double log_weight =
      trace.update(rng, gentl::change::no_change, proposed_values,
  ...
  double backward_log_density =
      mala_assess(previous_values, proposed_values,
                  trace.choice_gradient(selection), tau, storage2);

  double prob_accept = std::min(
      1.0, std::exp(log_weight + backward_log_density - forward_log_density));
  bool accept = std::bernoulli_distribution{prob_accept}(rng);
  if (!accept)
    trace.revert();
  return accept;
...
requires TopLevelGradientBasedChoiceUpdates<Trace, Selection, ChoiceBuffer> &&
    Revertible<Trace>
#endif
bool hmc(Trace &trace, const Selection &selection, size_t leapfrog_steps,
         double eps, ChoiceBuffer &momenta_buffer, ChoiceBuffer &values_buffer,
         RNG &rng) {
  ...
  for (size_t step = 0; step < leapfrog_steps; step++) {
    ...
    double log_weight_increment =
        trace.update(rng, gentl::change::no_change, values_buffer,
  ...
  double prob_accept = std::min(
      1.0, std::exp(log_weight + new_momenta_score - prev_momenta_score));
  bool accept = std::bernoulli_distribution{prob_accept}(rng);
  if (!accept)
    trace.revert();
  return accept;
